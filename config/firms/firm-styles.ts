...
export interface FirmStyleConfig {
  ...
  recruiterSummaryTone:
    'mbb_rigorous' | 'collegial_commercial' | 'competency_formal'
...
export const FIRM_CONFIGS: Record<FirmId, FirmStyleConfig> = {
  ...
  oliver_wyman: {
    ...
    firmFamily: 'boutique',
    tagline:
      'Quantitative depth · Financial services · Rigorous data interpretation',
    interviewLeadStyle: 'candidate_led',
    pacing: 'moderate',
    pushbackIntensity: 'medium',
    mathEmphasis: 'high',
...
export function firmIdFromFamily(family: string): FirmId | null {
  return (
    Object.values(FIRM_CONFIGS).find((c) => c.firmFamily === family)?.firmId ??
    null
  )
}
