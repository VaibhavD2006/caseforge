...
describe('computeLeaderboardScore', () => {
  it('returns zero when fewer than 3 eligible sessions', () => {
    expect(
      computeLeaderboardScore([card(8, 1), card(7, 2)]).leaderboardScore
    ).toBe(0)
  })
  ...
  it('caps at 3 sessions per day (anti-gaming)', () => {
    ...
    expect(
      computeLeaderboardScore(fiveSameDay).eligibleSessionCount
    ).toBeLessThanOrEqual(3)
  })

  it('excludes reliability-flagged sessions', () => {
    const flagged = [
      card(10, 1, 'evaluated', true),
      card(10, 2, 'evaluated', true),
      card(10, 3, 'evaluated', true),
    ]
