...
export async function getAdminOverviewStats() {
  ...
  const [totalUsers, activeSessions, allSessions, recentSessions] =
    await Promise.all([
      db.select({ count: sql<number>`count(*)` }).from(users),
      db
        .select({
          count: sql<number>`count(distinct ${interviewSessions.userId})`,
        })
  ...
  return {
    ...
    totalCostFormatted: formatCostDollars(
      Number(allSessions[0]?.totalCostCents ?? 0)
    ),
    avgScore: avgScoreRow[0]?.avg
      ? Number(avgScoreRow[0].avg).toFixed(2)
      : null,
  }
}
